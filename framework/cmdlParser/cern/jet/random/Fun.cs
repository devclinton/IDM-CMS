...
namespace cern.jet.random
{
	...
	/// <summary> Contains various mathematical helper methods.
	///
	...
	class Fun
	{
		...
		{
			throw new System.SystemException("Non instantiable");
		}
		...
		public static double bessel2_fkt(double lambda, double beta)
		{
			...
			double my, c, prod = 0.0, diff, value_Renamed;
			int i, j, nr_per;
			...
			if ((beta - 5.0 * lambda - 8.0) >= 0.0)
			{
				...
				{
					//while (!NULL) {
					if ((factorial(i) * System.Math.Pow((8.0 * beta), i)) > 1.0e250)
						break;
					if (i > 10)
						break;
					if (i == 1)
		...
		/// <summary> Modified Bessel Functions of First Kind - Order 0.</summary>
		public static double bessi0(double x)
		{
			...
			{
				y = 3.75 / ax;
				ans = (System.Math.Exp(ax) / System.Math.Sqrt(ax)) * (0.39894228 + y * (0.1328592e-1 + y * (0.225319e-2 + y * (- 0.157565e-2 + y * (0.916281e-2 + y * (- 0.2057706e-1 + y * (0.2635537e-1 + y * (- 0.1647633e-1 + y * 0.392377e-2))))))));
			}
			return ans;
		}
		/// <summary> Modified Bessel Functions of First Kind - Order 1.</summary>
		...
		/// <summary> Returns [tt]n![/tt].</summary>
		...
		/// <summary> Returns the gamma function [tt]gamma(x)[/tt].</summary>
		...
		/// <summary> Returns a quick approximation of [tt]log(gamma(x))[/tt].</summary>
