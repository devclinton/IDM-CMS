...
{
	/// <summary>MersenneTwister (MT19937) is one of the strongest uniform pseudo-random number generators known so far; at the same time it is quick.
	...
	/// </seealso>
	[Serializable]
	public class MersenneTwister:RandomEngine, System.ICloneable
	{
		...
		private static readonly int mag1 = MATRIX_A;
		//private static final int[] mag01=new int[] {0x0, MATRIX_A};
		/* mag01[x] = x * MATRIX_A  for x=0,1 */

		public const int DEFAULT_SEED = 4357;
		/// <summary> Constructs and returns a random number generator with a default seed, which is a [b]constant[/b].
		...
		public MersenneTwister(ref System.DateTime d):this((int) d.Ticks)
		{
		}
		/// <summary> Returns a copy of the receiver; the copy will produce identical sequences.
		...
		public override System.Object Clone()
		{
			...
			clone.mt = new int[this.mt.Length];
			this.mt.CopyTo(clone.mt, 0);
			return clone;
		}
		/// <summary> Generates N words at one time.</summary>
		protected internal virtual void  nextBlock()
		{
			...
			*/
			...
			for (kk = 0; kk < N - M; kk++)
			{
				...
				mt[kk] = mt[kk + M] ^ (SupportClass.URShift(y, 1)) ^ ((y & 0x1) == 0?mag0:mag1);
			}
			for (; kk < N - 1; kk++)
			{
				y = (mt[kk] & UPPER_MASK) | (mt[kk + 1] & LOWER_MASK);
				mt[kk] = mt[kk + (M - N)] ^ (SupportClass.URShift(y, 1)) ^ ((y & 0x1) == 0?mag0:mag1);
			}
			y = (mt[N - 1] & UPPER_MASK) | (mt[0] & LOWER_MASK);
			mt[N - 1] = mt[M - 1] ^ (SupportClass.URShift(y, 1)) ^ ((y & 0x1) == 0?mag0:mag1);
		...
		public override int nextInt()
		{
			...
			int y = mt[mti++];
			y ^= SupportClass.URShift(y, 11); // y ^= TEMPERING_SHIFT_U(y );
			...
			y ^= SupportClass.URShift(y, 18); // y ^= TEMPERING_SHIFT_L(y);
