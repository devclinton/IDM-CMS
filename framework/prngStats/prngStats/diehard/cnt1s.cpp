...
{
  const real mean=2500, std=sqrt(5000.0);
...
/* get a byte from a stream of bytes */
int GetBEx(IRandom *prng, int rt)
{
    static short rest=0;
    static uniform wd;
...
/* get a specific byte of a random integer */ 
int GetSbEx(IRandom *prng, int rt)
{
    return ( (prng->GetNext32() >> rt) & 255 );
}

/* count the 1's in the sequence of bytes */
real CountStatEx(IRandom *prng, counter no_wds, CountOnesTest test, int rshft)
{
    const real mean=2500, std=sqrt(5000.0);
    const real prob[]={37/256.,56/256.,70/256.,56/256.,37/256.};

    int (*byte)(IRandom *, int);
...
/*print the title*/
void CountTitleEx(IRandom *prng, CountOnesTest test)
{
    if( test == SPECIFIC ){
        puts("\n\t|-------------------------------------------------------------|");
        puts("\t|    This is the COUNT-THE-1''s TEST for specific bytes.      |");
...
void Count1sEx(IRandom *prng, CountOnesTest test)
{
  ...
  CountTitleEx(prng, test);
      
  printf("\t  (Degrees of freedom: 5^4-5^3=2500; sample size: %d)\n\n", no_wds);
  printf("\t%s\tchisquare\tz-score\t\tp-value\n", s);

  do{
    CountStatEx(prng, no_wds, test, rt);
    --rt;  
  }while(rt>=0);
...
char *CountOnesTestEnumNames[] = { "stream", "specific" };
