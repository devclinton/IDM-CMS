...
void SphereEx(IRandom *prng)
{
  ...
  for(i=1; i<=no_obs; ++i){ 
    ...
    for(j=0; j<no_pts; ++j){
/* clorton
      x[j]=ratio*prng->GetNext32();
      y[j]=ratio*prng->GetNext32();
      z[j]=ratio*prng->GetNext32();
*/
        x[j]=edge*prng->GetNextFloat();
        y[j]=edge*prng->GetNextFloat();
        z[j]=edge*prng->GetNextFloat();
    }
    qsort(x, no_pts, sizeof(real), fcmpr);
