...
unsigned long get_w(char *fn, short bits_pl, int rt)
{
  ...
  if(flag!=rt){
    flag=rt;
    maskltr = (1 << bits_pl) - 1; // pow(2,bits_pl)-1;
    ltrs_pw=20/bits_pl;
...
/*real*/ void monky_stat(char *filename, char *test, int no_tests)
{
  const real bits_pw=20, mean = (1 << (int)bits_pw) * exp(-2.0); // pow(2, bits_pw)*exp(-2.0);
  const counter dim = (1 << (int)(bits_pw-5)); // pow(2, bits_pw-5);

  register unsigned long j, u, l, wd;

  unsigned long maskbit[32];
  counter i, k, rt=0, bits_pl, no_wds = (1 << (int)(bits_pw+1)); // pow(2, bits_pw+1);
  ...
  for(j=0; j<32; ++j){
    maskbit[j] = (1 << j); // pow(2,j);
  }

  wds=(counter*)malloc(dim*sizeof(counter));

  do{
    for(i=1; (int)i<=no_tests; ++i){
      for(j=0; j<dim; ++j){
        wds[j]=0;  
      }
     
      for(j=1; j<=no_wds; ++j){
        wd = get_w(filename, (short)bits_pl, rt);
        l = wd & 31;
        u = wd >> 5; 
        wds[u] |= maskbit[l];
      }
...
/*print the title*/
void MonkeyTitleEx(IRandom *prng, MonkeyTest test)
{
    switch( test ){
    case OPSO:{
        puts("\n\t|-------------------------------------------------------------|");
    ...
    printf("\t\t\t   %s test for file %s\n\n", MonkeyTestEnumNames[test], prng->GetName());
    printf("\tBits used\tNo. missing words");
    printf("\tz-score\t\tp-value\n");
...
/* get a word from specific bits */
uint32_t GetWordEx(IRandom *prng, short bits_pl, int rt)
{
    static int flag=-1, ltrs_pw;
    static unsigned long wd, maskltr;
    short i;
...
void MonkeyStatEx(IRandom *prng, MonkeyTest test, int no_tests)
{
    const real bits_pw=20, mean = (1 << (int)bits_pw) * exp(-2.0); // pow(2, bits_pw)*exp(-2.0);
    const counter dim = (1 << (int)(bits_pw-5)); // pow(2, bits_pw-5);
    ...
    switch(test){
//    case 'I': rt=33; break;
    case OPSO: bits_pl=10; std=290; break;
    case OQSO: bits_pl=5;  std=295; break;
    case DNA: bits_pl=2;  std=339; break; 
    default  : break;
    }
    ...
    do{
        for(i=1; (int)i<=no_tests; ++i){
            ...
            for(j=1; j<=no_wds; ++j){
                wd = GetWordEx(prng, (short)bits_pl, rt);
                l = wd & 31;
                u = wd >> 5; 
                wds[u] |= maskbit[l];
            }
...
{
    const int no_tests = 1;
    MonkeyTitleEx(prng, test);
    MonkeyStatEx(prng, test, no_tests);
}

char *MonkeyTestEnumNames[] = { "OPSO", "OQSO", "DNA" };
