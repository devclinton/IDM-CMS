...
void DiehardEx(IRandom *prng)
{
    BDayEx(prng);
    Operm5Ex(prng);
    BinRankEx(prng, THIRTYONE_BY_THIRTYONE);
    BinRankEx(prng, THIRTYTWO_BY_THIRTYTWO);
    BinRankEx(prng, SIX_BY_EIGHT);
    BitTestEx(prng);
    MonkeyTestEx(prng, OPSO);
    MonkeyTestEx(prng, OQSO);
    MonkeyTestEx(prng, DNA);
    Count1sEx(prng, STREAM);
    Count1sEx(prng, SPECIFIC);
    ParkEx(prng);
    MinDistEx(prng);
    SphereEx(prng);
    SqueezeEx(prng);
