...
void osum(char *filename)
{
  ...
  const real mean=.5*no_num, rstd=sqrt(12.0);
...
void OSumEx(IRandom *prng)
{
  ...
  for(i=1; i<=no_obs; ++i){
   ...
   /* get a sequence of numbers and add them up */
      sum=0;
      for(k=0; k<no_num; ++k){
// clorton        y[k]=prng->GetNext32()/UNIMAX;
        y[k]=prng->GetNextFloat();
        sum+=y[k];
      }
   ...
   /* discard the first number in the sequence and get another number
      ...
      for(k=1; k<no_num; ++k){
        tmp=y[k-1];
        y[k-1]=(sum-mean)*rstd;
// clorton        sum-=(tmp-prng->GetNext32()/UNIMAX);
        sum-=(tmp-prng->GetNextFloat());
      }
      y[no_num-1]=(sum-mean)*rstd;
