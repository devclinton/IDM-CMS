...
real P_fit(real lambda, counter *obs, int no_obs)
{
  ...
  for(j=0; j<dim; ++j){
    ...
    while( ((int)k < no_obs) && ((int)obs[k]<=i) ){
      ++f[j];
      ++k;
    }
...
void bday(char *filename)
{
  ...
  register const counter mask=(1 << no_bits) - 1; // pow(2,no_bits)-1;
  const real lambda= (real)(1 << 30) / (1 << (no_bits+2)); // pow(no_bday,3)/(4*pow(2,no_bits));
...
#define GETADAY       ( (prng->GetNext32() >> rt) & mask )
