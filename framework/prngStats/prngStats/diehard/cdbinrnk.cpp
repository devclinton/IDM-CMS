...
real rnk_stat(char *fn, counter no_mtr, char *test, int rt)
{
  ...
  switch(test[1]){
    case '1': bm.no_row=bm.no_col=31; bm.mask=0x7FFFFFFF; llim=28; p=p30; break;
    case '2': bm.no_row=bm.no_col=32; bm.mask=0xFFFFFFFF; llim=29; p=p30; break;
    default: {
       df=2;
       bm.no_row=6;
       bm.no_col=8; 
       bm.mask= 0xFF; // pow(2,8)-1;
       llim=4;
       p=p6;
       printf("\n\t\t\t      bits %2d to %2d\n", 25-rt, 32-rt);
       break;
...
/*print the title*/
void RankTitleEx(IRandom *prng, BinRankTest test)
{
    switch (test) {
    case SIX_BY_EIGHT:
        puts("\n\t|-------------------------------------------------------------|");
    ...
    printf("\t\tRank test for binary matrices (%s) from %s\n", BinRankTestEnumNames[test], prng->GetName());

    return;
}

/* perform the test and calculate test-stat */
real RankStatEx(IRandom *prng, counter no_mtr, BinRankTest test, int rt)
{
    real p6[]={.009443, 0.217439, 0.773118};
    real  p30[]={.0052854502, .1283502644, .5775761902, .2887880952};
    ...
    switch(test){
    case THIRTYONE_BY_THIRTYONE: bm.no_row=bm.no_col=31; bm.mask=0x7FFFFFFF; llim=28; p=p30; break;
    case THIRTYTWO_BY_THIRTYTWO: bm.no_row=bm.no_col=32; bm.mask=0xFFFFFFFF; llim=29; p=p30; break;
    default: {
        df=2;
        bm.no_row=6;
        bm.no_col=8; 
    ...
    for( i=1; i<=no_mtr; ++i ){
        ...
        for(j=0; (int)j<bm.no_row; ++j){
            switch(test){ 
            case THIRTYONE_BY_THIRTYONE: bm.row[j]=( prng->GetNext32()>>1 ); break;
            case THIRTYTWO_BY_THIRTYTWO: bm.row[j]=prng->GetNext32(); break;
            default: bm.row[j]=( prng->GetNext32()>>rt ); break;
            }
            bm.row[j] &= bm.mask;
        } 
...
void BinRankEx(IRandom *prng, BinRankTest test)
{
    ...
    RankTitleEx(prng, test);

    p=(real*)malloc( (rt+1)*sizeof(real) );

    do{
        p[rt]=RankStatEx(prng, no_matrices, test, rt);
        --rt; 
    }while(rt>=0);
...
char *BinRankTestEnumNames[] = { "6x8", "31x31", "32x32" };
