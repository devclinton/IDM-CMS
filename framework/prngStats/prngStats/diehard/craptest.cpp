...
void craptest(char *filename)
{
  ...
  for(i=1; i<20; ++i){
    f[i]=0;
    Ef[i]=(27*pow(27/36.0,(int)i-1)+40*pow(26/36.0,(int)i-1)+55*pow(25/36.0,(int)i-1))/648;
    sum+=Ef[i];
  }
  Ef[20]=1.-sum;
  f[0]=f[20]=0;
...
void CrapTestEx(IRandom *prng)
{
    ...
    /*playing the games*/
#undef DIE
// clorton #define DIE   1 + (int)( 6* (prng->GetNext32()/UNIMAX) )
#define DIE   (1 + (int)(6 * prng->GetNextFloat()))
#ifndef GAME_OVER
#define GAME_OVER  {++f[MIN(20,no_throw)]; no_win+=win;}
#endif
