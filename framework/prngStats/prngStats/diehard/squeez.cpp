...
void squeez(char *filename)
{
  ...
  const real ratio=no_trials/1000000., std=sqrt(84.0);
  ...
  for(i=1; i<=no_trials; ++i){
    ...
    /* squeeze k */
    while( (k!=1)&&(j<48) ){
      k=(unsigned long)(k*( uni(filename)/UNIMAX )+1);
      ++j;
    }
...
void SqueezeEx(IRandom *prng)
{
    ...
    for(i=1; i<=no_trials; ++i){
        ...
        /* squeeze k */
        while( (k!=1)&&(j<48) ){
// clorton            k=(unsigned long)(k*( prng->GetNext32()/UNIMAX )+1);
            k=(unsigned long)(k*( prng->GetNextFloat() )+1);
            ++j;
        }
