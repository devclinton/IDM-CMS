...
void bitst(char *filename)
{
  ...
  const counter no_bits= (1 << (power-5)); // pow(2,power-5);
  const counter no_wds = (1 << power); // pow(2,power);
  const counter dim= (1 << (nb_pw-5)); // pow(2,nb_pw-5);
  register counter mask= (1 << nb_pw) - 1; // pow(2,nb_pw)-1; 
  const real mean= (1 << nb_pw) * exp((double)(-(1 << (power-nb_pw)))); // pow(2,nb_pw)*exp(-pow(2,(power-nb_pw)));
  const real std=428;
