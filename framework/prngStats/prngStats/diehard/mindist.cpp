...
#undef GETPT
// clorton #define GETPT   { pts[j].y=ratio*prng->GetNext32(); pts[j].x=ratio*prng->GetNext32(); }
#define GETPT   { pts[j].y = 10000.0f * prng->GetNextFloat(); pts[j].x = 10000.0f * prng->GetNextFloat(); }
    for(i=1; i<=no_smpl; ++i){
        dmin=2*side*side;
        for(j=0; j<no_pts; ++j){
            GETPT
