...
main(int argc, char *argv[])
{
    /*
    {
        /* Time for 10^6 uniform random numbers. */
        uint32_t seedData[] = { 0x0BADBEEF, 0xBEEFDEAD, 0x19680201, 0x20120913 };
        IRandom *prng = RandomFactory::CreatePRNG(PRNG_AESCTR, seedData, sizeof(seedData));
#define nFLOATS (1048576)
        float *uniform = new float[nFLOATS];
        uint64_t startCount;
        uint64_t endCount;
        uint64_t counterFrequency;
        QueryPerformanceCounter((LARGE_INTEGER*)&startCount);
        prng->GetFloats(uniform, nFLOATS);
        QueryPerformanceCounter((LARGE_INTEGER*)&endCount);
        QueryPerformanceFrequency((LARGE_INTEGER*)&counterFrequency);
        printf("%d uniform floats generated in %I64u counts (%I64u counts/second)\n", nFLOATS, (endCount - startCount), counterFrequency);

        float *normal = new float[nFLOATS];
        float *src = uniform;
        float *dst = normal;
        QueryPerformanceCounter((LARGE_INTEGER*)&startCount);
        for (int i = 0; i < nFLOATS; i += 2)
        {
            float root = sqrt(-2.0f * log(*src++));
            float angle = 6.2831853f * *src++;
            *dst++ = root * cos(angle);
            *dst++ = root * sin(angle);
        }
        QueryPerformanceCounter((LARGE_INTEGER*)&endCount);
...
#define nDOUBLES (nFLOATS * 16)
        double *ee = new double[nDOUBLES];
        prng->GetBits((void *)ee, nDOUBLES * sizeof(double));
        uint64_t *bits = (uint64_t *)ee;
        for (int i = 0; i < nDOUBLES; i++)
        {
            union { double d; uint64_t u; } t;
            t.d = 1.0;
            t.u |= (*bits++ & 0x000FFFFFFFFFFFFF) | 0x0000000000000001;
            t.d -= 1;
            ee[i] = t.d;
        }
        normal = new float[nFLOATS];
        double *pee = ee;
        dst = normal;
        QueryPerformanceCounter((LARGE_INTEGER*)&startCount);
        for (int i = 0; i < nFLOATS; i += 2)
    ...
    */
    ...
    if ( argc < 2 ) {
        printf("Usage: %s <stream length> <prng> [test mask]\n", argv[0]);
        printf("   <stream length> is the length of the individual bit stream(s) to be processed\n");
        printf("   <prng> is one of the supported PRNGs:\n");
        printf("       EMODLCG - linear congruential\n");
        printf("       RANDLIB - RANDLIB generator\n");
        printf("       PDES    - pseudo-DES\n");
        printf("       MT      - Mersenne Twister (original implementation)\n");
        printf("       MT64    - Mersenne Twister (64-bit implementation)\n");
        printf("       TINYMT  - Tiny Mersenne Twister\n");
        printf("       SFMT    - SIMD Fast Mersenne Twister (using SSE2)\n");
        printf("       AESCTR  - AES (counter mode)\n");
        return 0;
    }
...
//    option = generatorOptions(&streamFile);
    option = DeterminePRNG(argc >= 3 ? argv[2] : nullptr, &streamFile);
    chooseTests(argc >= 4 ? atoi(argv[3]) : 0);
    if (argc < 3) fixParameters();
    openOutputStreams(option, (argc >= 5) ? atoi(argv[4]) : 0);
...
{ 
    ...
    sprintf_s(resultsDir, 200, "experiments/%s/%s/results.txt", generatorDir[option], testNames[testNameID]);

    // if ( (fp[numOfFiles] = fopen(resultsDir, "r")) == NULL ) {
    if ( fopen_s(&fp[numOfFiles], resultsDir, "r") != 0 ) {
        printf("%s", resultsDir);
    ...
    for ( i=0; i<numOfFiles; i++ ) {
        if ( i < 10 )
            ...
            sprintf_s(s[i], 200, "experiments/%s/%s/data%1d.txt", generatorDir[option], testNames[testNameID], i+1);
        else if (i < 100)
            // sprintf(s[i], "experiments/%s/%s/data%2d.txt", generatorDir[option], testNames[testNameID], i+1);
            sprintf_s(s[i], 200, "experiments/%s/%s/data%2d.txt", generatorDir[option], testNames[testNameID], i+1);
        else
            // sprintf(s[i], "experiments/%s/%s/data%3d.txt", generatorDir[option], testNames[testNameID], i+1);
            sprintf_s(s[i], 200, "experiments/%s/%s/data%3d.txt", generatorDir[option], testNames[testNameID], i+1);
    }
    numread = 0;
    m = numOfFiles/20;
    if ( (numOfFiles%20) != 0 )
            ...
            if ( ((i == TEST_CUSUM) && testVector[TEST_CUSUM] ) ||
                ...
                for ( k=0; k<numOfFiles; k++ ) {
                    if ( k < 10 )
                    {
                        ...
                        sprintf_s(s, 200, "experiments/%s/%s/data%1d.txt", generatorDir[option], testNames[i], k+1);
                    }
                    else if ( k < 100 )
                    {
                        // sprintf(s, "experiments/%s/%s/data%2d.txt", generatorDir[option], testNames[i], k+1);
                        sprintf_s(s, 200, "experiments/%s/%s/data%2d.txt", generatorDir[option], testNames[i], k+1);
                    }
                    else
                    {
                        // sprintf(s, "experiments/%s/%s/data%3d.txt", generatorDir[option], testNames[i], k+1);
                        sprintf_s(s, 200, "experiments/%s/%s/data%3d.txt", generatorDir[option], testNames[i], k+1);
                    }
                ...
                sprintf_s(s, 200, "experiments/%s/%s/results.txt", generatorDir[option], testNames[i]);
                generalSampleSize = computeMetrics(s,i);
            }
        }
    }
