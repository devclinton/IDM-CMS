...
void IRandomWrapper32(char *title, char *id, PRNG_TYPE type, void *seedData, size_t seedSize)
{
    printf("%s...\n", title);

    // Allocate epsilon for n bits
    if ( (epsilon = (BitSequence *) calloc(tp.n, sizeof(BitSequence))) == NULL ) {
        printf("[%s] Insufficient memory available (requested %d).\n", id, tp.n * sizeof(BitSequence));
        exit(1);
    }

    uint32_t *randomVariates;
    if ((randomVariates = (uint32_t*)calloc(tp.n >> 5, sizeof(uint32_t))) == NULL) {
        printf("[%s] Insufficient memory available (requested %d).\n", id, (tp.n >> 5) * sizeof(uint32_t));
        free(epsilon);
        exit(1);
    }
    ...
    IRandom *prng = RandomFactory::CreatePRNG(type, seedData, seedSize);

    for (int i = 0; i < tp.numOfBitStreams; i++) {
        int cOnes = 0;
        cBits     = 0;
        uint32_t *pBits = randomVariates;
        ...
        prng->GetBits(randomVariates, tp.n >> 3);
        QueryPerformanceCounter((LARGE_INTEGER*)&endCount);
        QueryPerformanceFrequency((LARGE_INTEGER*)&counterFrequency);
        double bitsPerSecond = tp.n * counterFrequency / double(endCount - startCount);
        printf("\t\t%12.0f bits per second (%d * %I64u / %I64u)\n", bitsPerSecond, tp.n, counterFrequency, endCount - startCount);
        fprintf(freqfp, "\t\t%12.0f bits per second (%d * %I64u / %I64u)\n", bitsPerSecond, tp.n, counterFrequency, endCount - startCount);
#endif

        // Store n bits in epsilon
        for (int j = 0; j < tp.n; j++) {
            ...
            cOnes += (epsilon[j] = bits & 1);
            bits >>= 1;
            cBits--;
        }

        fprintf(freqfp, "\t\tBITSREAD = %d 0x = %d 1s = %d\n", tp.n, tp.n - cOnes, cOnes); fflush(freqfp);
        nist_test_suite();
        DiehardEx(prng);
    }
...
{
    uint32_t seed = 1968;
    IRandomWrapper32("EMOD Linear Congruential Generator", "EmodLcg", PRNG_EMODLCG, &seed, sizeof(seed));
}
...
void RandLib()
{
    uint32_t seeds[] = { 0x31415926, 0x27182818 };
    IRandomWrapper32("RANDLIB random number generator", "RandLib", PRNG_RANDLIB, seeds, sizeof(seeds));
/*
    printf("RANDLIB...\n");

    if ( (epsilon = (BitSequence *)calloc(tp.n, sizeof(BitSequence))) == NULL) {
        printf("[RandLib] Insufficient memory available (requested %d).\n", tp.n * sizeof(BitSequence));
        exit(1);
    }
    ...
    for (int i = 0; i < tp.numOfBitStreams; i++) {
        ...
        for (int j = 0; j < tp.n; j++) {
            ...
            if (cBits == 0) {
                int temp = Common::GenerateLargeInteger();
                bits = (*(unsigned int *)&temp) >> 8;
                cBits = 24;
            }
...
{
    uint32_t seed = 0;
    IRandomWrapper32("EMOD Pseudo-DES Generator", "PseudoDES", PRNG_EMODPDES, &seed, sizeof(seed));
}

#include "..\twister\original\mt19937ar.h"
...
void MersenneTwister()
{
    ...
    if ( (epsilon = (BitSequence*)calloc(tp.n, sizeof(BitSequence))) == NULL ) {
        printf("[MersenneTwister] Insufficient memory available (requested %d).\n", tp.n * sizeof(BitSequence));
        exit(1);
    }

    uint32_t *randomVariates;
    if ((randomVariates = (uint32_t*)calloc(tp.n >> 5, sizeof(uint32_t))) == NULL) {
        printf("[MersenneTwister] Insufficient memory available (requested %d).\n", (tp.n >> 5) * sizeof(uint32_t));
        free(epsilon);
        exit(1);
    }
...
void IRandomWrapper64(char *title, char *id, PRNG_TYPE type, void *seedData, size_t seedSize)
{
    printf("%s...\n", title);
    ...
    uint64_t *randomVariates;
    if ((randomVariates = (uint64_t *)calloc(tp.n >> 6, sizeof(uint64_t))) == NULL) {
        printf("[%s] Insufficient memory available (requested %d).\n", id, (tp.n >> 6) * sizeof(uint64_t));
        free(epsilon);
        exit(1);
    }

printf("randomVariates = 0x%16p\n", randomVariates);
...
void MT64()
{
    uint64_t seed = 0x0BADC0DEED0CDAB0;
    IRandomWrapper64("Mersenne Twister 64", "MT64", PRNG_MT64, &seed, sizeof(seed));
}

#include "..\twister\tinymt\tinymt64.h"
...
void TinyMT()
{
    ...
    if ( (epsilon = (BitSequence*)calloc(tp.n, sizeof(BitSequence))) == NULL ) {
        printf("[TinyMT] Insufficient memory available (requested %d).\n", tp.n * sizeof(BitSequence));
        exit(1);
    }

    uint64_t *randomVariates;
    if ((randomVariates = (uint64_t*)calloc(tp.n >> 6, sizeof(uint64_t))) == NULL) {
        printf("[TinyMT] Insufficient memory available (requested %d).\n", (tp.n >> 6) * sizeof(uint64_t));
        free(epsilon);
        exit(1);
    }
    ...
    tinymt64_t state;
    tinymt64_init(&state, 0x0BADC0DEED0CDAB0);
    ...
    for (int i = 0; i < tp.numOfBitStreams; i++ ) {
        ...
        uint64_t    counterFrequency;
        QueryPerformanceCounter((LARGE_INTEGER*)&startCount);
        for (int j = 0; j < (tp.n >> 6); j++)
            *pBits++ = tinymt64_generate_uint64(&state);
        QueryPerformanceCounter((LARGE_INTEGER*)&endCount);
        QueryPerformanceFrequency((LARGE_INTEGER*)&counterFrequency);
        double bitsPerSecond = tp.n * counterFrequency / double(endCount - startCount);
        printf("\t\t%12.0f bits per second (%d * %I64u / %I64u)\n", bitsPerSecond, tp.n, counterFrequency, endCount - startCount);
...
void SFMT()
{
    uint32_t seed[] = { 0x0BADC0DE, 0xF00DBEEF, 0x03212012, 0x12212012 };
    IRandomWrapper64("SIMD Fast Mersenne Twister", "SFMT", PRNG_SFMT, seed, sizeof(seed));
}

void AES_CTR()
{
    uint64_t seed = 1968;
    IRandomWrapper64("AES Counter Mode", "AESCTR", PRNG_AESCTR, &seed, sizeof(seed));
}
