...
Serial(int m, int n)
{
    ...
    p_value1 = cephes_igamc((1 << (m-1))/2, del1/2.0);   // cephes_igamc(pow(2, m-1)/2, del1/2.0);
    p_value2 = cephes_igamc((1 << (m-2))/2, del2/2.0);   // cephes_igamc(pow(2, m-2)/2, del2/2.0);
...
psi2(int m, int n)
{
    ...
    if ( (m == 0) || (m == -1) )
        return 0.0;
    numOfBlocks = n;
    powLen = (1 << (m+1)) - 1;    // (int)pow(2, m+1)-1;
