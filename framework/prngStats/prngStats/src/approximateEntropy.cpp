...
ApproximateEntropy(int m, int n)
{
    ...
    for ( blockSize=m; blockSize<=m+1; blockSize++ ) {
        if ( blockSize == 0 ) {
            ...
            powLen = (1 << (blockSize+1))-1;    // (int)pow(2, blockSize+1)-1;
            ...
            /* DISPLAY FREQUENCY */
            sum = 0.0;
            index = (1 << blockSize)-1; // (int)pow(2, blockSize)-1;
    ...
    p_value = cephes_igamc((1 << (m-1)), chi_squared/2.0);  // cephes_igamc(pow(2, m-1), chi_squared/2.0);
