function CEDBIR_v4_2(k_default, Kce_default, re_val, re_type, fixed_index, irr_index, bin_index, rho_default, lambda, ic, eps_ce_vec, eps_final_vec, alpha, PathName, FileName)
% CEDBIR
    ...
    for m = 1: len_eps_final_vec
        ...
        while(iter < max_iter)
            ...
            parfor i=1:Kce
                ...
                [localMax, minRelProp, maxRelProp]  = solveOnce_v4_2(ic, k, gamma, prop_cutoff, alpha, bin_index, [fixed_index irr_index], re_val, re_type);
                x_ext(i) = localMax;
                maxRP(:,i) = maxRelProp';
                minRP(:,i) = minRelProp';
            end
            ...
            while (iter2 < max_iter2)
                ...
                for i=1:length(bin_index)
                    blen = length(prop_cutoff{bin_index(i)})+1;
...
mfile = [FileName, '_gammaMat_BA'];%[file_in, '_lambda', lambdastr,'_N',num2str(expo)];
save(fullfile(PathName, FileName, mfile), 'gammas', 'Kce', 'rho', 'runTimes', 'numIters', 'eps_ce_vec', 'eps_final_vec', 'convTable', 'KceTable','counter');
end
