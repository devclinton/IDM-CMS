...
figure, imagesc(Channel2D); title('Vassel geometry');
...
figure, imagesc(Obstacles); title(' Fluid obstacles (in the fluid)' );
% 
Medial_axis=bwmorph(Channel2D,'thin',Inf); %
figure, imagesc(Medial_axis); title('Medial axis');
...
while(~StopFlag)
   ...
   % uyout(ija)=uyout(ija)./rho(ija); % from momentum to velocity
    ...
    if (mod(Cur_Iter,Output_Every)==0) ;  % Output from loop every ...
        ...
        rho=sum(f,3); % density
        figure(10); imagesc(rho,[0.1 0.9]); title(' rho'); % visualize density evolution
        figure(11); imagesc(ux,[ ]); title(' ux' ); % visualize fluid velocity horizontal
        figure(12); imagesc(-uy,[ ]); title(' uy' ); % visualize fluid velocity down
        figure(14), imagesc(-uyout,[]), title('uyout'); % vis vel flow out
