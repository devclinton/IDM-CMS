classdef EQFScales
    ...
    methods
        ...
        function macro = restrict(obj, currentMacro, dataSet)
            ...
            NonZero = Wild>0;
            MeanVal = mean(log(dataSet.data(:,NonZero)),2);
            StdVal = std(log(dataSet.data(:,NonZero)), 0, 2);
            NewZero = sum(~NonZero);
        ...
        function ICList = lift(obj, macro, NTrajectories)
            ...
            meanvals(isnan(meanvals)|isinf(meanvals)) = 0;
            stdvals(isnan(stdvals)|isinf(stdvals)) = 0;
            for ii = 1:obj.NSpecies
                ICList(ii,:) = exp(meanvals(ii) + stdvals(ii)*randn(1,NTrajectories));
            end
