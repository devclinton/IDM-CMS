...
Tau-Leaping is a stochastic simulation algorithm (SSA) developed by Gillespie to increase the computational speed of the exact SSAs [1]_.  Instead of computing the time to every reaction, this algorithm approximates the process and attempts to leap in time, executing a large number of reactions in a period tau.  This algorithm is computationally faster, however, the approximation removes the “exact” connection to the solution of the master equation for the system.

The implementation of tau-leaping in this framework is based on a recent work by Cao et. al [2]_.  This modified tau-leaping algorithm helps avoid the possibility of creating negative species counts within a compartment.
