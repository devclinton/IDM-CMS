...
# (c) 2016 Institute for Disease Modeling
#
# Usage:
#  pythong gti.py [-i <sourceDir>] [-o <indexFileJs>] [-u <urlPrefix>]
#
# Where:
...
def process_file(file):
    ...
    # get text
    docDiv = soup.body.find_all("div", class_="document")
    if len(docDiv) > 0:
        # print(str.format("len(docDiv) = {}", len(docDiv)))
        text = docDiv[0].get_text(separator=' ')
        text = text.replace("\n", "")
        text = re.sub('\s+', " ", text)
        text = text.strip()
    else:
...
count = len(files)
if count == 0:
    print(str.format('Error: no HTML files found at {}', args.input))
    sys.exit()
